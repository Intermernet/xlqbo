...
func serveProducts(w http.ResponseWriter, r *http.Request, cred *oauth.Credentials, company string) {
        var products map[string]interface{}
        if err := callAPI(
                cred,
                company,
                "query",
                url.Values{"query": {"select * from Item"}},
                &products); err != nil {
                http.Error(w, "Error getting products, "+err.Error(), 500)
                return
        }
        respond(w, productsTmpl, products)
}

var httpAddr = flag.String("addr", ":9999", "HTTP server address")
...
func main() {
	...
	http.Handle("/products", &authHandler{handler: serveProducts})
...
<{{.}}
        ...
        productsTmpl = template.Must(template.New("products").Parse(
                `<html>
<head>
</head>
<body>
<p><a href="/">home</a>
{{range .QueryResponse.Item}}<p>{{.Name}}{{end}}
<{{.}}
</body></html>`))
)
